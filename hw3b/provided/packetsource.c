...
PacketSource_t * createPacketSource(long mean, int numSources, short seed) {
	...
	packetSource->numSources = numSources;
	...
	packetSource->constantMean = mean;
	packetSource->constantSeed = (UniformGenerator_t *)malloc(sizeof(UniformGenerator_t) * numSources);
    ...
    for( int i = 0; i < numSources; i++) {
	  ...
	  initGenerator(&(packetSource->constantSeed[i].randGen));
      packetSource->constantSeed[i].maxValue= (BIG_NUM + 1);
      packetSource->constantSeed[i].randGen.seed = ((1+i) & 0xFF);
      ...
      for( short j = 0; j < seed; j++ ) {
		...
		getUniformRand(&(packetSource->constantSeed[i]));
      }
    }
    return packetSource;
}
void deletePacketSource(PacketSource_t * packetSource)
{
	...
	free(packetSource->constantSeed);
	free(packetSource->constantCounts);
...
volatile Packet_t * getUniformPacket(PacketSource_t * packetSource, int sourceNum){
	...
	packetSource->uniformCounts[sourceNum] += 1;
	return tmp;
}
volatile Packet_t * getExponentialPacket(PacketSource_t * packetSource, int sourceNum)
{
	...
	packetSource->exponentialCounts[sourceNum] += 1;
	return tmp;
}
volatile Packet_t * getConstantPacket(PacketSource_t * packetSource, int sourceNum) {
	volatile Packet_t * tmp = (volatile Packet_t *)malloc(sizeof(volatile Packet_t));
	tmp->iterations = packetSource->constantMean;
	tmp->seed = getUniformRand(&(packetSource->constantSeed[sourceNum]));
	packetSource->constantCounts[sourceNum] += 1;
	return tmp;
}
...
long getConstantCount(PacketSource_t * packetSource,int sourceNum) {
	return packetSource->constantCounts[sourceNum];
}
